...
public class Character : MonoBehaviour {
	...
	void Update ()
    {
        if (currentPos == nextPos && GameManager.routeFound) // if the next destination has been reached, and if there is a route to walk
        {
     //      SetNextPos(); // this probably should not be in update
        }
        if (canMove)
        {
            Move();
    ...
    {
        if (GameManager.RoutePoints.Count != 1) // can prolly remove this if-statement
        {
        ...
        else
        {
            ...
            {
                // if all minerals have been found
